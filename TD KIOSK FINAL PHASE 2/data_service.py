...
def _storage_dir_candidates() -> List[pathlib.Path]:
    """Ordered storage roots: env/config path, internal USB, app fallback."""
    seen = set()
...
def _storage_dir_score(path: pathlib.Path) -> int:
    """Prefer a tree that already has persisted factory settings."""
...
def record_failed_login(username: str, *, apply_lock: bool = True) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        fa = 0
    fa += 1
    if apply_lock and fa >= 3 and status == "active":
        status = "locked"
    m["failedAttempts"] = fa
    m["status"] = status
    _save_member_record(m)
